...
def get_camera_ip(mac_add: str) -> str:
    while True:
        try:
            output = subprocess.check_output(f"arp -a | findstr \"{mac_add}\"", shell=True)
            ip = output.decode('utf-8').split()[0]
            logging.info(f'IP: {ip}')
            return ip
        except subprocess.CalledProcessError:
            logging.error(f'Unable to get IP for MAC address: {mac_add}. Retrying in 1 second...')
            time.sleep(1)
...
rtsp_handler(username='admin', password='L28FA3F4', mac_add='e4-24-6c-16-d4-f0', plt=True)
