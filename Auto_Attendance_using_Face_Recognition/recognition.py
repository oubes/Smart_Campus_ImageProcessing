...
class face_recognizer(ABC):
    ...
    def Recognize(self, unlabeled_img_url):
        ...
        for ID in IDs:
            for labeled_encoded_face in list(self.encoded_dict[ID].values()):
    ...
    def add_labeled_encoded_entry(self, labeled_face_url: str, ID: str) -> dict[str, dict]:
        # Add check for config
        if ID in self.encoded_dict.keys():
            if labeled_face_url in self.encoded_dict[ID].keys() and not(self.recognizer_config.encodingUpdate):
        ...
        self.encoded_dict[ID] = {}
        self.encoded_dict[ID][labeled_face_url] = labeled_encoded_face
        return self.encoded_dict
    ...
    @abstractmethod
    def compare_faces(self, labeled_face_encoded_img, unlabeled_face_encoded_imgs, threshold) -> Tuple[List[bool], np.ndarray]:
        pass
