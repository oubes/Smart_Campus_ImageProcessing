...
def adjust_brightness_contrast(image, brightness_factor, contrast_factor):
    if config["PerprocessingConfig"]["FilterEable"][
        "enable_brightness_contrast_adjustment"
    ]:
...
def apply_gaussian_blur(image):
    if config["PerprocessingConfig"]["FilterEable"]["enable_gaussian_blur"]:
        ...
        kernel_size = (
            int(
                np.random.uniform(
                    config["PerprocessingConfig"]["KERNAL"]["lowest_gaussian_blur"],
                    config["PerprocessingConfig"]["KERNAL"]["highest_gaussian_blur"],
                )
            )
            * 2
            + 1
...
def apply_color_jittering(image):
    if config["PerprocessingConfig"]["FilterEable"]["enable_color_jittering"]:
        ...
        gamma = np.random.uniform(
            config["PerprocessingConfig"]["GAMMA"]["lowest_color_jittering"],
            config["PerprocessingConfig"]["GAMMA"]["highest_color_jittering"],
        )
        saturation = np.random.uniform(
            config["PerprocessingConfig"]["SATURATION"]["lowest_saturation"],
            config["PerprocessingConfig"]["SATURATION"]["highest_saturation"],
        )
...
def image_augmentation(image):
    ...
    brightness_factor = np.random.uniform(
        config["PerprocessingConfig"]["BRIGHTNESS"]["lowest_brightness"],
        config["PerprocessingConfig"]["BRIGHTNESS"]["highest_brightness"],
    )
    contrast_factor = np.random.uniform(
        config["PerprocessingConfig"]["CONTRAST"]["lowest_contrast"],
        config["PerprocessingConfig"]["CONTRAST"]["highest_contrast"],
    )
    adjusted_image = adjust_brightness_contrast(
        image, brightness_factor, contrast_factor
    )
    # Apply Gaussian blur
    ...
    # flip_brightness
    flip_brightness = adjust_brightness_contrast(
        flipped_image, brightness_factor, contrast_factor
    )
    # flip_histo
    ...
    # brighness_noise
    brightness_noise = adjust_brightness_contrast(
        noisy_image, brightness_factor, contrast_factor
    )
    # brighness_histo
    brightness_histo = adjust_brightness_contrast(
        histo_image, brightness_factor, contrast_factor
    )
    # brighness_gaussian
    brighness_gaussian = adjust_brightness_contrast(
        blurred_image, brightness_factor, contrast_factor
    )
    # gaussian_jittered
